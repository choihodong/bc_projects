const rooms = ['KNN', 'MBC', 'RODONG']
exports.creatChatServer = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('s_send_userName', (user_name, time) => {
            ...
            //기존방에 있던 멤버들에게 방송:나를 제외한 모든 멤버
            socket.broadcast.to(socket.room)
                .emit('c_send_msg', '관리자', `${socket.user_name}님이 이방에 입장하였습니다.`, new Date())
            //방금 조인은 유저에세 반송
        ...
        //클라리언트가 방을 바꿔달라고 했다, 변경하고 정보 전송
        socket.on('s_send_roomChg',(newRoom)=>{
            //1.기존방에서 떠난다.
            socket.leave(socket.room)
            //2.기존방 멤버들에게 방송
            socket.broadcast.to(socket.room)
                .emit('c_send_msg', '관리자', `${socket.user_name}님이 이방에 퇴장하였습니다.`, new Date())
            //3.새로운 방 진입 절차 시작
            socket.room=newRoom
            //3-1 방정보 설정
            socket.join(socket.room)
