...
- 데이터베이스 1년간 무료 지원( )
- 가입 후 > 콘솔에 로그인 > 서비스 > ec2클릭시 화면이 열리면 승인완료
1) 우측 상단 > zone 조절 > 아시아 태평양(서울) 조정
 ...
 > putty.exe 실행 > ubunt@본인서버공인IP > saved Session에 이름 기입(aws) > save
 > Connection > SSH > Auth > Browse 클릭 > xxx.ppk 선택 > session > aws선택 > save > 
...
4.nodejs
 ...
 - 일반유저가 블록체인에 다이렉트로 접근해서 업무를 볼수없으므로,
   ...
   - 개념
    > V8엔진 기반 구동
    > 비동기, 이벤트 기반
    > 비슷한 유형 : 루비, 파이썬 분야의 웹 서비스
    > 1 스레드로 동시에 만건 요청을 처리했다 ?
    > nodejs vs spring : 어떤것을 사용할 것인가? 각각 환경에 맞게 구성
   ...
   - 실시간 통신
    ...
    - 배포 및 운영
     ...
     소스 업로드, node_modules, package-lock.json 을 뺀 나머지 전부
     $ cd notJapan
     $ npm install 
     $ nano package.json
     ------------------------------
     ...
     $ pm2 restart 0
     => 서비스 종료, 완전제거 => 더이상 운영하지 않겠다 : pm2 delete name|id
     $ pm2 delete 0
     --------------------------------------------------------------------
     ...
     $ tail -f ./logs/out.log
     --------------------------------------------------------
     => 사업이 잘되서 동접요청이 폭주를 했다. -> 현 서버에서 성능을 극대화 할 것 인가? => 클러스터
